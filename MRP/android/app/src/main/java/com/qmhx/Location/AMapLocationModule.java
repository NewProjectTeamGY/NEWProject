...
public class AMapLocationModule extends ReactContextBaseJavaModule implements AMapLocationListener, GeocodeSearch.OnGeocodeSearchListener {
    ...
    public void onLocationChanged(AMapLocation amapLocation) {
        if (amapLocation != null) {
            map = amapLocationToObject(amapLocation);
            if (map.getInt("errorCode") > 0) {
                sendEvent("onAmapLocationChanged", map);
                map = null;
            } else {
                if (!TextUtils.isEmpty(map.getString("city")) && !TextUtils.isEmpty(map.getString("locationDetail"))) {
                    sendEvent("onAmapLocationChanged", amapLocationToObject(amapLocation));
                    map = null;
                } else {
                    map = Arguments.createMap();
                    map.putString("latitude", String.format("%.6f", amapLocation.getLatitude()));
                    map.putString("longitude", String.format("%.6f", amapLocation.getLongitude()));
                    GeocodeSearch geocoderSearch = new GeocodeSearch(mReactContext);
    ...
    public void onRegeocodeSearched(RegeocodeResult regeocodeResult, int i) {
        if (i == 1000) {
            amapRegeocodeSearchedToObject(regeocodeResult.getRegeocodeAddress());
        } else {
            map.putBoolean("isSucceed", false);
    ...
    private WritableMap amapRegeocodeSearchedToObject(RegeocodeAddress regeocodeAddress) {
        map.putBoolean("isSucceed", true);
        map.putString("adCode", regeocodeAddress.getAdCode());
        map.putString("province", regeocodeAddress.getProvince());
        map.putString("address", regeocodeAddress.getFormatAddress());
        map.putString("city", regeocodeAddress.getCity());
        map.putString("cityCode", regeocodeAddress.getCityCode());
        map.putString("district", regeocodeAddress.getDistrict());
        return map;
    }
    ...
    private WritableMap amapLocationToObject(AMapLocation amapLocation) {
        ...
        if (errorCode > 0) {
            ...
            map.putString("latitude", String.format("%.6f", latitude));
            map.putString("longitude", String.format("%.6f", longitude));
